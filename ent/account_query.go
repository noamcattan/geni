...
func (aq *AccountQuery) QueryMember() *UserQuery {
...
func (aq *AccountQuery) QueryCategories() *CategoryQuery {
...
func (aq *AccountQuery) Clone() *AccountQuery {
	...
	return &AccountQuery{
		...
		withMember:     aq.withMember.Clone(),
...
func (aq *AccountQuery) WithMember(opts ...func(*UserQuery)) *AccountQuery {
	query := &UserQuery{config: aq.config}
...
func (aq *AccountQuery) WithCategories(opts ...func(*CategoryQuery)) *AccountQuery {
	query := &CategoryQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := aq.withMember; query != nil {
		if err := aq.loadMember(ctx, query, nodes,
			func(n *Account) { n.Edges.Member = []*User{} },
			func(n *Account, e *User) { n.Edges.Member = append(n.Edges.Member, e) }); err != nil {
			return nil, err
		}
	}
	if query := aq.withCategories; query != nil {
		if err := aq.loadCategories(ctx, query, nodes,
			func(n *Account) { n.Edges.Categories = []*Category{} },
			func(n *Account, e *Category) { n.Edges.Categories = append(n.Edges.Categories, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AccountQuery) loadMember(ctx context.Context, query *UserQuery, nodes []*Account, init func(*Account), assign func(*Account, *User)) error {
	...
	query.Where(predicate.User(func(s *sql.Selector) {
		s.Where(sql.InValues(account.MemberColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.account_member
		if fk == nil {
			return fmt.Errorf(`foreign-key "account_member" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "account_member" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (aq *AccountQuery) loadCategories(ctx context.Context, query *CategoryQuery, nodes []*Account, init func(*Account), assign func(*Account, *Category)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(account.CategoriesTable)
		s.Join(joinT).On(s.C(category.FieldID), joinT.C(account.CategoriesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(account.CategoriesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(account.CategoriesPrimaryKey[0]))
