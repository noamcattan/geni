...
func (cq *CategoryQuery) QueryExpenses() *ExpenseQuery {
	query := &ExpenseQuery{config: cq.config}
...
func (cq *CategoryQuery) QueryAccount() *AccountQuery {
...
func (cq *CategoryQuery) WithAccount(opts ...func(*AccountQuery)) *CategoryQuery {
	query := &AccountQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Category, error) {
	...
	if query := cq.withAccount; query != nil {
		if err := cq.loadAccount(ctx, query, nodes,
			func(n *Category) { n.Edges.Account = []*Account{} },
			func(n *Category, e *Account) { n.Edges.Account = append(n.Edges.Account, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CategoryQuery) loadExpenses(ctx context.Context, query *ExpenseQuery, nodes []*Category, init func(*Category), assign func(*Category, *Expense)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(category.ExpensesTable)
		s.Join(joinT).On(s.C(expense.FieldID), joinT.C(category.ExpensesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(category.ExpensesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(category.ExpensesPrimaryKey[0]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "expenses" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (cq *CategoryQuery) loadAccount(ctx context.Context, query *AccountQuery, nodes []*Category, init func(*Category), assign func(*Category, *Account)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Category)
	nids := make(map[int]map[*Category]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(category.AccountTable)
		s.Join(joinT).On(s.C(account.FieldID), joinT.C(category.AccountPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(category.AccountPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(category.AccountPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
