...
func (*User) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case user.ForeignKeys[0]: // account_member
			values[i] = new(sql.NullInt64)
		default:
...
func (u *User) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case user.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field account_member", value)
			} else if value.Valid {
				u.account_member = new(int)
				*u.account_member = int(value.Int64)
			}
		}
	}
	return nil
